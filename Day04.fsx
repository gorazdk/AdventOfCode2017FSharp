let input = """nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
|> Array.filter (fun s -> Array.length(s) = Array.length(Array.distinct(s)))
|> Array.countBy(fun t -> "")

let sortWord (input:string) =
  let sorted = 
    input
    |> Seq.sort
  
  System.String.Join("", sorted)
...
input.Split('\n')
|> Array.map (fun s -> s.Split(' '))
|> Array.map (fun s -> s |> Array.map(fun p -> sortWord p))
|> Array.filter (fun s -> Array.length(s) = Array.length(Array.distinct(s)))
|> Array.length
